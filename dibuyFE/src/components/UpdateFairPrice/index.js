...
const UpdateFairPrice = () => {
  ...
  const onChangeProduct = (event) => {
    ...
    const filteredObj = categoryProducts.filter(
  ...
  const renderForm = () => (
    ...
    >
      ...
      <TextField
        ...
        variant="standard"
        className="w-100"
        helperText="Please enter Dealer price"
        onChange={(event) => setDealerPrice(event.target.value)}
        value={dealerPrice}
        required
      />
      <Button
