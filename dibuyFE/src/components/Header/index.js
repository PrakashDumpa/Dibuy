...
const navItems = [
  { item: "Products", id: 1, path: "/products" },
  { item: "Seller Corner", id: 2, path: "/sellercorner" },
  { item: "Cart", id: 3, path: "/cart" },
];
    ...
    >
      <List>
        {[
          ...
          {
            displayText: "Orders",
            icon: <GrHistory className="h5 m-0" />,
            path: "/orders",
          },
          {
            displayText: "ChatUs",
            icon: <BsChatRightDotsFill className="h5 m-0" />,
            path: "/chatus",
          },
          {
            displayText: "DashBoard",
            icon: <MdSell className="h5 m-0" />,
            path: "/seller/dashboard",
          },
          {
            displayText: "Fair Price",
            icon: <BiRupee className="h5 m-0" />,
            path: "/fair-price",
          },
          {
            displayText: "Admin",
            icon: <MdAdminPanelSettings className="h5 m-0" />,
            path: "/admin",
          },
        ].map((obj, index) => (
      ...
      <Divider />
      <List>
        {[{ displayText: "Logout", icon: <FiLogOut className="h5 m-0" /> }].map(
          (obj, index) => (
          ...
          <>
              ...
              <div className="navParentCon d-flex justify-content-center text-white">
                <div className="navbarCon d-flex justify-content-between">
                  <h1 className="pt-2 pb-2">
                    ...
                    >
                      <span className="websiteNativeColor">Di</span>Buy
                    </Link>
                  </h1>
                  <div className="d-flex">
                    <ul className="list-unstyled d-flex m-0">
                      {navItems.map((eachItem) => (
                        <li
                          className={
                            currentRoute === eachItem.path
                              ? "mr-3 websiteNativeBgColor p-2 d-flex align-items-center navItem justify-content-center"
                              : "mr-3 d-flex align-items-center navItem justify-content-center"
                          }
                          key={eachItem.id}
                          onClick={() => setCurrentRoute(eachItem.path)}
                        >
                          <Link to={eachItem.path} className="navLink">
                            <span>{eachItem.item}</span>
                            {eachItem.item === "Cart" && renderCart()}
                          </Link>
                        </li>
                      ))}
                    </ul>
                    <button
                      ...
                      type="button"
                      onClick={toggleDrawer(openingSide, true)}
                    >
                      More{" "}
                      <FcNext className="text-light m-0 font-weight-bold" />
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="d-md-none">
              <nav className="navbar navbar-light bg-light">
                ...
                <div className="">
                  ...
                  >
                    {" "}
                    <RiAccountCircleFill className="text-info h2 m-0 font-weight-bold" />
                  </button>
                </div>
            ...
            <Dialog open={open}>
              <div className="logoutCon d-flex flex-column justify-content-between">
                ...
                <small className="text-secondary text-center w-100">
                  Logging you out...
                </small>
              </div>
            </Dialog>
